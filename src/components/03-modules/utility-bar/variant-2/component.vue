<template>
  <div :class="classes">
    ...
    <Container>
      ...
      <a :href="buildNavLink(this.backLink)" class="link"><Icon graphic="cheveron" placement="before" /><span class="link_text">{{ this.backLinkText }}</span></a>

      <SearchBar :class="this.searchBarState == 'expanded' ? 'expanded' : 'collapsed'" @click="toggleSearch()" v-if="this.hasSearch" />
...
<script>
  ...
  import SearchBar              from '@/components/03-modules/search-bar/variant-1/component'


  export default {
    name: 'UtilityBar',
    components: {
      Container, SearchBar, Link, Icon
    },
    props: {
      classList:{
        type: String,
    ...
    computed: {
      classes() {
        return `utility-bar ${this.classList}`;  
      }
    },
    methods: {
      ...
      toggleSearch() {
        if(this.searchBarState != "expanded"){
          this.searchBarState = "expanded";
        }
      }
    }
  }
