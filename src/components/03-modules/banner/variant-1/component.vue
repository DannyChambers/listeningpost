<template>
  ...
  <Pane variant="vertical" :backgroundImage="this.backgroundImage" :class="classes">
    <Container>
...
<script>
  ...
  export default {
    name: 'Banner',
    components: {
      Listen, Container, Layout, PageHeader, ProductList, Logo, PrimaryNavigation, Accordion, ProgressIndicator, PageFooter, Pane, Form, LoginForm, Legend, Card, CopyBlock, H1, H2, H3,H4, H5, T1, T2, T3,
    ...
    computed: {
      classes() {
        ...
        if(this.backgroundImage){
          classes = `banner banner--${this.variant} banner--dark ${this.classList}`
        } else {
          classes = `banner banner--${this.variant} ${this.classList}`
        }
